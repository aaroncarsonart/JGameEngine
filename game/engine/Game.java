...
import game.engine.mode.ElevationMapMode;
import game.engine.mode.GameMode;
import game.engine.mode.LineGraphMode;
import game.graphics.GameDisplayPanel;
import game.graphics.GameFrame;
import game.graphics.GameGraphics;
import game.map.MapMode;
...
public class Game implements Runnable, Serializable
{
	...
	private void testElevationMapMode() {
		double[][] values = TerrainGenerator.diamondSquareAlgorithm(2, -100,
				100, 0.5);
	 ...
	 */
	private void testMapMode() {
		setGameMode(new MapMode());
	}
	...
	public void start() {
		try {
			...
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					displayPanel = new GameDisplayPanel(currentMode
							.getGameGraphics());
